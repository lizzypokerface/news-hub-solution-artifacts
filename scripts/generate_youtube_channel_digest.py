...
try:
    ...
    print(
        "Please run: pip install google-api-python-client python-dateutil langchain langchain_community",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _get_channel_id_from_url(url, youtube_service):
    ...
    if not match:
        print(
            f"    -  Could not parse handle from URL: {url}. Only '@handle' URLs are supported."
        )
        return None
...
if __name__ == "__main__":
    ...
    API_KEY = "AIzaSyBAmGmUB5hQh9FO-DxFwLyVznT_QFxDsK0"  # <--- PASTE YOUR KEY HERE

    if API_KEY == "YOUR_API_KEY":
        print(
            "ERROR: Please replace 'YOUR_API_KEY' with your actual YouTube Data API Key in the script.",
            file=sys.stderr,
        )
        sys.exit(1)
