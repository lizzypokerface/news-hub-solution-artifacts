...
try:
    ...
    print(
        "Please run: pip install PyYAML google-api-python-client python-dateutil langchain langchain_community",
        file=sys.stderr,
    )
    sys.exit(1)
...
class ConfigManager:
    ...
    def load_config(self):
        ...
        try:
            ...
            print(
                f"FATAL: Configuration file not found at '{self.config_path}'",
                file=sys.stderr,
            )
            return False
...
def _get_channel_id_from_url(url, youtube_service):
    ...
    try:
        ...
        return (
            search_response["items"][0]["id"]["channelId"]
...
if __name__ == "__main__":
    ...
    if not API_KEY or "AIzaSyxxxxxxxx" in API_KEY:
        print(
            "FATAL: YouTube API key is missing or is a placeholder in the config file.",
            file=sys.stderr,
        )
        sys.exit(1)
...
---
**EXAMPLE OF DESIRED LOGIC AND OUTPUT:**
...
"""
        ...
        print(
            "Please ensure Ollama is running and your API key is valid.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not datapoint_sources:
        print(
            "No sources with type 'datapoint' and format 'youtube' found in config file."
        )
        sys.exit(0)
    ...
    for source in datapoint_sources:
        ...
        event_summary = synthesize_events_from_titles(
            source["name"], video_titles, llm_chain
        )
    ...
    output_filename = f"what_world_is_saying_{datetime.now().strftime('%Y-%m-%d')}.json"
    ...
    try:
        ...
        print(
            f"FATAL: Could not write to output file '{output_filepath}'. Error: {e}",
            file=sys.stderr,
        )
